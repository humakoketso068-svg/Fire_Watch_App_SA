...
from geo import (
    ESWATINI, LABELLED, LESOTHO, PROVINCE_LABELS, SA_OUTLINE, project,
)
...
class FireMap(ft.Container):
    ...
    def _polygon(self, latlon_points, fill, scale, ox, oy):
        """A filled polygon with a thin outline, from (lat, lon) points."""
        pts = [
            self._pos(*project(lat, lon), scale, ox, oy)
            for lat, lon in latlon_points
        ]
        elements = [cv.Path.MoveTo(*pts[0])]
        elements += [cv.Path.LineTo(x, y) for x, y in pts[1:]]
        elements.append(cv.Path.Close())
        return [
            cv.Path(elements, ft.Paint(color=fill, style=ft.PaintingStyle.FILL)),
            cv.Path(
                elements,
                ft.Paint(color=LAND_EDGE, style=ft.PaintingStyle.STROKE, stroke_width=1),
            ),
        ]

    def _terrain_controls(self, scale, ox, oy):
        """South Africa outline, the Lesotho / eSwatini enclaves and province names."""
        key = (round(self.avail_w), round(self.avail_h))
        if key == self._bg_key:
            return list(self._bg_controls)

        shapes = self._polygon(SA_OUTLINE, LAND, scale, ox, oy)
        shapes += self._polygon(LESOTHO, MAP_BG, scale, ox, oy)
        shapes += self._polygon(ESWATINI, MAP_BG, scale, ox, oy)
        ...
        if self.VIEW_W * scale >= 480:
            for name, lat, lon in PROVINCE_LABELS:
                px, py = self._pos(*project(lat, lon), scale, ox, oy)
                controls.append(
                    ft.Container(
                        content=txt(name.upper(), 8.5, PROVINCE_TEXT),
                        left=px - 50,
                        top=py - 6,
                        width=100,
        ...
        for zone in state["zones"]:
            ...
            controls.append(
                ft.Container(
                    ...
                    height=nr * 2,
                    bgcolor=PANEL,
                    border=ft.Border.all(1.4, RISK[zone["risk_level"]]),
                    border_radius=nr,
                )
            )
            if show_labels and zone["name"] in LABELLED:
                controls.append(
                    ft.Container(
...
class ZoneRow(ft.Container):
    ...
    def render(self, zone, wind_speed):
        ...
        self.meta.value = (
            f'{zone["province"]} · {round(zone["temp"])}°F, '
            f'{round(zone["moisture"])}% moisture, wind {round(wind_speed)} mph'
        )
            ...
            for fire in active:
                rows.append(
                    ft.Container(
                        content=ft.Row(
                            [
                                txt(f'{fire["zone_name"]}, {fire["province"]}', 14, TEXT),
    ...
    def build_ops(self):
        ...
        map_panel = card(
            ft.Column(
                [
                    ft.Row(
                        [txt("South Africa sensor map", 13, TEXT, bold=True),
